(function($) {
    $.fn.scrupulous = function(args) {
        ...
        if (typeof document.createElement("input").checkValidity != "function") {
            $forms.on("submit." + options.namespace,
                function(e) {
            ...
            if (!$el.is(":hidden")) {
                ...
                if ($formGroup.length === 0) {
                    ...
                    if ($formGroup.length === 0) {
                        if (window.console) {
                            console.log(
                                "Warning: Scrupulous needs a .form-group, .input-group or parentClassName element to append errors. id: " +
                                $el.attr("id"));
                        }
                        return false;
                    }
                ...
                if (typeof errorMessage === "undefined" ||
                    ...
                    errorMessage === originalValidationMessage) {
        ...
        $inputs.each(function() {
            ...
            $(this).on("blur." + options.namespace,
                function() {
            ...
            if (! $(this).is("select")) {
                ...
                $(this).on("change." +
                    ...
                    " mouseup." +
                    options.namespace,
                    function() { validityEventCheck(this); });

            } else {
                // If we are a select, only bind change and input.. not the others, some browsers are cranky
                $(this).on("change." + options.namespace + " input." + options.namespace,
                    function() { validityEventCheck(this); });
            }
