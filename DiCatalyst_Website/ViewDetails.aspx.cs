...
public partial class ViewDetails : Page
{
    ...
    protected void load_DoctorImages()
    {
        ...
        if (!string.IsNullOrEmpty(Session["doctorid"] as string))
        {
            ...
            var query = "Select images from doctorsdata where uniqueRandomId ='" + _doctorid + "'";
            ...
            {
                string[] images = dr.ItemArray[0].ToString().TrimStart(';').Split(';');

                for (int i = 1; i <= images.Length; i++)
                {
                    ContentPlaceHolder cp = this.Master.FindControl("ContentPlaceHolder1") as ContentPlaceHolder;
                    var img = (Image)cp.FindControl("HspImg" + i);

                    if (!string.IsNullOrEmpty(images[i - 1]))
                        img.ImageUrl = "~\\Dentist\\HospitalImages\\" + images[i - 1];
                }
            }
            con.Close();
        }
    ...
    private void LoadAppointments()
    {
       // var id = Session["doctorid"].ToString(); // basic validation
        if (!string.IsNullOrEmpty(Session["doctorid"] as string))
        {
           // _appointments = LoadAppointmentsbyUserLoginId(Session["doctorid"].ToString(),DateTime.Now,DateTime.Now.AddDays(30));
            _appointments = _addPatient.getDoctorAppointmentForNonRegistered(Session["doctorid"].ToString(), DateTime.Now, DateTime.Now.AddDays(30));
    ...
    {
        var da = new SqlDataAdapter(
            "SELECT * FROM [Appointment] WHERE ([AppointmentStatus] = 'free') AND NOT (([AppointmentEnd] <= @start) OR ([AppointmentStart] >= @end)) union  select * from Appointment where AppointmentPatientId = @UserId",
            ConfigurationManager.ConnectionStrings["dicatalyst"].ConnectionString);
        da.SelectCommand.Parameters.AddWithValue("@UserId", loginUserId);
        da.SelectCommand.Parameters.AddWithValue("start", start);
        da.SelectCommand.Parameters.AddWithValue("end", end);
        var dt = new DataTable();
        da.Fill(dt);
