...
(function($) {
    ...
    //Dientes izquierdos
    for (var i = 0; i < 8; i++) {
        dientes.push(new DienteModel(1 + i, i * 25, 0));
    }
    for (var i = 0; i < 5; i++) {
        dientes.push(new DienteModel(1 + i, (i + 3) * 25, 1 * 40));
    }
    for (var i = 0; i < 5; i++) {
        dientes.push(new DienteModel(20 - i, (i + 3) * 25, 2 * 40));
    }
    for (var i = 0; i < 8; i++) {
        dientes.push(new DienteModel(32 - i, i * 25, 3 * 40));
    }
    //Dientes derechos
    for (var i = 0; i < 8; i++) {
        dientes.push(new DienteModel(9 + i, i * 25 + 210, 0));
    }
    for (var i = 0; i < 5; i++) {
        dientes.push(new DienteModel(6 + i, i * 25 + 210, 1 * 40));
    }
    for (var i = 0; i < 5; i++) {
        dientes.push(new DienteModel(15 - i, i * 25 + 210, 2 * 40));
    }
    for (var i = 0; i < 8; i++) {
        dientes.push(new DienteModel(24 - i, i * 25 + 210, 3 * 40));
    }
    ...
    function drawDiente(svg, parentGroup, diente) {
        ...
        var caraCompleto = svg.text(dienteGroup,
            6,
            30,
            diente.id.toString(),
            {
        ...
        var tratamientosAplicadosAlDiente = arrayFilter(tratamientosAplicados,
            function(t) {
    ...
    var methods = {
        ...
        removeTratamiento: function(tratamiento) {
            tratamientosAplicados = arrayFilter(tratamientosAplicados,
                function(t) {
    ...
    $.fn.odontograma = function(method) {
        ...
        if (methods[method]) {
            ...
            $.error("Method " + method + " does not exist on jQuery.odontogram");
        }
    };
})(jQuery);